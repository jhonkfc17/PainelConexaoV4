-- Suporte ao tipo JUROS no RPC principal de pagamentos.
-- Retrocompatibilidade: formatos antigos (ADIANTAMENTO_MANUAL + flags) continuam válidos.
...
begin
  ...
  if p_tipo not in ('PARCELA_INTEGRAL','ADIANTAMENTO_MANUAL','JUROS','SALDO_PARCIAL','QUITACAO_TOTAL','DESCONTO') then
  ...
  if p_tipo = 'PARCELA_INTEGRAL' then
    ...
    -- Pagamento de juros não amortiza principal da parcela.
    update public.parcelas
      set updated_at = now()
    where id = v_parcela.id;
