...
function composicaoJurosPagar(emprestimo: any, p: ReturnType<typeof normalizeParcela>, dataPagamentoISO: string) {
  ...
  const totalComposto = Math.max(0, Math.min(jurosAtraso + multaAplicada + diferencaEmprestado, limiteParcela));
...
export default function RegistrarPagamentoModal({ open, onClose, onSaved, emprestimo }: Props) {
  ...
  const composicaoJurosDaParcela = (p: ReturnType<typeof normalizeParcela>) => composicaoJurosPagar(emprestimo, p, dataPagamento);
  const composicaoJurosRef = useMemo(
    () => (parcelaRef && MODO_JUROS ? composicaoJurosDaParcela(parcelaRef) : null),
    [parcelaRef, emprestimo, dataPagamento]
  );

  useEffect(() => {
    if (!open || !MODO_JUROS) return;
    const alvo = parcelaRef ?? parcelasAbertas[0] ?? null;
    if (!alvo) {
      setValor(0);
      return;
    }
    setValor(composicaoJurosPagar(emprestimo, alvo, dataPagamento).totalComposto);
  }, [open, parcelaRef, parcelasAbertas, emprestimo, dataPagamento]);

  const saldoSelecionado = useMemo(() => {
    if (parcelasSelecionadas.length === 0) return 0;
    if (MODO_JUROS) return parcelasSelecionadas.reduce((acc, p) => acc + composicaoJurosDaParcela(p).totalComposto, 0);
          ...
          flags: {
            ...
            juros_composto: MODO_JUROS
              ? {
                  ...
                  multa_aplicada: Number(compJuros?.multaAplicada ?? 0),
                  diferenca_sobre_emprestado: Number(compJuros?.diferencaEmprestado ?? 0),
                  total_calculado: Number(compJuros?.totalComposto ?? 0),
                }
              : undefined,

            amort_preview: !MODO_JUROS && amortPreview
              ? {
      ...
      <div className="relative z-10 w-[92vw] max-w-[560px] max-h-[92vh] overflow-hidden rounded-2xl border border-white/10 bg-[#0B1312] shadow-2xl flex flex-col">
        ...
        <div className="p-5 sm:p-6 pb-4 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h2 className="text-xl font-semibold text-white">{MODO_JUROS ? "Registrar Pagamento de Juros" : "Registrar Pagamento"}</h2>
        ...
        {/* tabs */}
        <div className="mt-4">
          <div className="text-sm text-white/70 mb-2">{MODO_JUROS ? "Tipo de Pagamento (Juros)" : "Tipo de Pagamento"}</div>
          <div className="grid grid-cols-4 gap-2">
            {(MODO_JUROS
              ? ([["PARCIAL", "Juros"]] as const)
              : ([
                  ["PARCELA", "Parcela"],
                  ["PARCIAL", "Parcial"],
                  ["TOTAL", "Total"],
              ...
              <button
                key={k}
                onClick={() => {
                  if (MODO_JUROS && k !== "PARCIAL") return;
                  setTab(k);
                }}
                disabled={MODO_JUROS && k !== "PARCIAL"}
                className={
                  `rounded-xl px-3 py-2 text-xs sm:text-sm border ${MODO_JUROS ? "col-span-4" : ""} ` +
                  (tab === k
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <div className="text-sm text-white">{brl(MODO_JUROS ? composicaoJurosDaParcela(p).totalComposto : totalDaParcela(emprestimo, p, dataPagamento))}</div>
        ...
        {tab === "PARCIAL" || tab === "DESCONTO" ? (
          <div className="mt-4 grid grid-cols-1 gap-3">
                ...
                {MODO_JUROS ? (
                  <>
                    <div className="mt-1 flex items-center justify-between text-sm text-white/70">
                      ...
                      <b className="text-white">{brl(Number(composicaoJurosRef?.jurosAtraso ?? 0))}</b>
                    </div>
                    <div className="mt-1 flex items-center justify-between text-sm text-white/70">
            ...
            <div>
              <label className="mb-1 block text-sm text-white/80">{MODO_JUROS ? "Valor do pagamento de juros *" : tab === "DESCONTO" ? "Valor do desconto *" : "Valor Pago *"}</label>
              <input
                type="number"
                value={valor || ""}
